...
sub run ($self) {
    ...
    # the first successful login prompt. expect_not_found records the absence
    # as a successful serial assertion; an undef result means a fatal marker
    # appeared during the bounded observation window.
    my $quiet_boot = wait_serial($fatal, timeout => 90, expect_not_found => 1);
    die 'Doors boot gate observed a fatal serial signature after boot completion' unless defined $quiet_boot;
}
