...
Both bots use GitHub’s protected PR merge path; GitHub completes a merge only after the protected `policy`, `image`, and `dependency-review` checks pass. Dependabot requests native auto-merge unless GitHub reports a validated PR as already clean, in which case its trusted helper uses the SHA-bound ordinary squash PR merge that GitHub protects equivalently. They open/rebase/merge PRs rather than directly pushing `main`; failures, upstream package incompatibilities, and unresolved checks remain fail-closed. Install the free Renovate GitHub App for this repository and keep its scope restricted to this repository. See [`AUTONOMOUS-MAINTENANCE.md`](AUTONOMOUS-MAINTENANCE.md).
