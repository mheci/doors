...
3. `publish` uses checksum-verified Trivy in serial image mode to create the SPDX SBOM from that immutable digest, then uploads GitHub OIDC build-provenance and SBOM attestations to the registry. Its scanner credential/cache is isolated under `RUNNER_TEMP`; a separate GHCR-only Docker login exists only for the registry-attestation actions and is logged out even after failure. Trivy is used because Syft's layer-tree squashing exceeds standard hosted-runner memory on large Fedora bootc images; this changes the scanner, not the immutable subject or attestation requirement. A failure in any of those steps fails the `publish` release gate.
