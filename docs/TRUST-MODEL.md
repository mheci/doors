...
3. `publish` uses checksum-verified Trivy in serial image mode to create the SPDX SBOM from that immutable digest, then uploads GitHub OIDC build-provenance and SBOM attestations to the registry. Trivy is used because Syft's layer-tree squashing exceeds standard hosted-runner memory on large Fedora bootc images; this changes the scanner, not the immutable subject or attestation requirement. A failure in any of those steps fails the `publish` release gate.
