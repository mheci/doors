...
| Bazaar | Bazzite native `flatpak-preinstall.service` + Flathub | Doors explicitly enables the native first-boot service and owns the sole `bazaar.preinstall` descriptor. The native Flatpak resolver installs only `io.github.kolunmi.Bazaar` and its required runtime extensions; it does not use BlueBuild's separate `default-flatpaks` manager. |
