...
| Terra 44 | Gaming/performance packages, Zen, Vicinae, Ghostty, Zed, Bun, Deno, mise, and OpenCode CLI | Vendored complete GPG fingerprint set; `gpgcheck=1`, `repo_gpgcheck=1`, no availability bypass. |
| npm registry | Native Pi and `t3` CLIs, including their Linux x86_64 platform packages | Tracked locks pin exact HTTPS tarballs and SRI SHA-512 identities; compose uses `npm ci --ignore-scripts --omit=dev`, so package lifecycle code cannot run. |
...
All supported development and AI tools are native image content. DNF composes Node/npm/pnpm, Python/pip, C/C++ build tools, Bun, Deno, mise, OpenCode CLI, and the signed `cuda-toolkit-13-4` source route before the image is signed. Tracked Pi and `t3` locks add their native CLIs through exact HTTPS tarball URLs and SRI SHA-512 digests; `npm ci` verifies each lock and runs with lifecycle scripts disabled. The fixed toolkit major receives only NVIDIA CUDA 13.4 updates; it does not bring a driver route into the image. In the same signed-RPM compose layer, its vendor `/usr/local/cuda-13.4` payload and `/opt/nvidia` Nsight payloads are copied to immutable `/usr/lib/doors/cuda-13.4` and removed from mutable sources; `/usr/bin/nvcc`, `/usr/bin/ncu`, and `/usr/bin/nsys` are stable command paths.

Herdr is the sole CI-generated executable input. CI resolves its immutable GitHub release asset, verifies the release SHA-256 and `gh release verify-asset` attestation, then passes only that ephemeral artifact and manifest to the compose. The image validates the repository, release tag, asset name, and digest again before installing `/usr/bin/herdr`.
