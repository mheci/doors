...
# unrelated beta/nightly trust material and an updater. Doors uses only the
# reviewed Origin keys, has no runtime Brave repo, and updates through rebuilt
# images, so remove the unused material after the strict DNF transaction.
set -euo pipefail
...
# The package's updater only re-imports every RPM-GPG-KEY-brave* file. Remove
# it so future image layers cannot recreate unrelated key trust; image updates
# always compose a new signed image instead of mutating this deployment.
