#!/usr/bin/env bash
# Sign every shipped kernel PE/COFF payload and loadable kernel module with the
# Doors Machine Owner Key during composition.
#
...
# Set only when this RUN installed headers solely to obtain sign-file. It is
# removed before the layer commits.
...
require_readable_file() { [[ -r "$1" && -f "$1" ]] || die "required file is missing or unreadable: $1"; }
...
key_public_sha256="$(openssl pkey -in "${mok_key}" -pubout -outform DER 2>/dev/null | sha256sum | awk '{print $1}')"
...
sign_pe_payload() {
  local payload="$1" replacement
  replacement="$(mktemp "${scratch}/signed-payload.XXXXXX")"
  rm -f -- "${replacement}"
  sbsign --key "${mok_key}" --cert "${certificate_pem}" --output "${replacement}" "${payload}"
...
sign_module() {
  local sign_file="$1" module="$2" expanded replacement signer
  ...
  [[ "${signer}" == "${expected_signer}" ]] || die "MOK module verification failed for ${module}"
}
...
while IFS= read -r -d '' payload; do
  ...
  # Fedora ships the ordinary vmlinuz plus optional UKI, DTB-loader, and addon
  # PE/COFF artifacts below module-version trees. Sign all EFI payloads there
  # rather than relying on a package-name-specific subset.
...
install_transient_kernel_devel() {
  ...
  if rpm -q kernel-devel >/dev/null 2>&1; then return 0; fi
...
resolve_sign_file() {
  local kernel_dir="$1" kernel_version="$2" candidate fallback=''
  # Prefer a precisely matched tool if the base carries it. BlueBuild bases can
  # intentionally lead Fedora metadata, so a standalone kernel-devel copy is a
  # safe fail-closed fallback: sign-file's module-signature format is
  # version-independent.
  for candidate in \
    "${kernel_dir}/build/scripts/sign-file" \
    "${kernel_source_root}/${kernel_version}/scripts/sign-file"; do
    if [[ -x "${candidate}" ]]; then sign_file="${candidate}"; return 0; fi
  done

  if [[ -d "${kernel_source_root}" ]]; then
    while IFS= read -r candidate; do fallback="${candidate}"; done < <(
      find "${kernel_source_root}" -type f -path '*/scripts/sign-file' -perm /111 -print | LC_ALL=C sort
    )
  fi

  # Do not retain the large header package in a separate layer: install it only
  # after the exact/base-provided paths are exhausted, then remove it.
  if [[ -z "${fallback}" ]]; then
    install_transient_kernel_devel
    while IFS= read -r candidate; do fallback="${candidate}"; done < <(
      find "${kernel_source_root}" -type f -path '*/scripts/sign-file' -perm /111 -print | LC_ALL=C sort
  ...
  [[ -n "${fallback}" ]] || die "kernel-devel sign-file is unavailable for kernel ${kernel_version}"
  sign_file="${fallback}"
}
...
while IFS= read -r -d '' kernel_dir; do
  ...
  # Refresh dependency metadata only for actual kernel module trees.
  first_module="$(find "${kernel_dir}" -type f \( \
    -name '*.ko' -o -name '*.ko.xz' -o -name '*.ko.zst' -o -name '*.ko.gz' \
  \) -print -quit)"
  if [[ -n "${first_module}" ]]; then depmod -a "${kernel_version}"; fi
